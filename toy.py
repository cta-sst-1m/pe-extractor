...
def n_photon_from_rate(photon_rate_mhz, n_bin=800, bin_size_ns=0.5):
    """
    get number of photon per time bin
    :param photon_rate_mhz: photon rate in mhz
    :param n_bin: number of time bins to return
    :param bin_size_ns: duration of a time bin
    :
    :return: array of number of photon per time bin (size=(n_bin, batch_size))
    """
    rate_per_bin = photon_rate_mhz * bin_size_ns / 1000
    # the number of photons in a time bin follow a Poisson distribution
    n_photon_per_bin = np.random.poisson(rate_per_bin, n_bin)
    return n_photon_per_bin
...
def prepare_pulse_template(
    ...
    :param bin_size_ns: duration of a time bin for the photons
    ...
    # interpolate the template to the photon binning
...
def waveform_from_n_photon(
        n_photon_per_bin, template_amplitude_bin, n_bin_per_sample=8,
        noise_lsb=1.05
):
    """
    get waveform from an array containing the number of photons per time bin
    :param n_photon_per_bin: array containing the number of photons per time
    bin
    :param template_amplitude_bin: array containing pulse template ready to be
    convolved, f.e. from prepare_pulse_template()
    :param n_bin_per_sample: number of time bin (for photons) per sample
    :param noise_lsb: amplitude in lsb of the electronic noise
    :return: waveform corresponding to the n_photon_per_bin array
    """
    # convolve the number of photon to the template to get the samples
    # (with the photon binning size)
    waveform_bin = np.convolve(n_photon_per_bin, template_amplitude_bin, 'same')
    # integrate waveform bin to get the wanted sampling rate
    n_sample = int(np.floor(len(n_photon_per_bin) / n_bin_per_sample))
...
def _get_batch(
        batch_size, n_sample, n_bin_per_sample, photon_rate_mhz, bin_size_ns,
    ...
    n_photon_batch = np.zeros([batch_size, n_bin - n_bin_init])
    waveform_batch = np.zeros([batch_size, n_sample - n_sample_init])
    if np.size(photon_rate_mhz) == 1:
        photon_rate_batch = np.ones(batch_size) * photon_rate_mhz
    elif np.size(photon_rate_mhz) == 2:
        # rates are chosen from the range of photon rate
        rate_min = np.min(photon_rate_mhz)
        rate_max = np.max(photon_rate_mhz)
        length_interval = rate_max - rate_min
        photon_rate_batch = np.random.random(batch_size) * length_interval + \
                            rate_min
    else:
        raise NotImplementedError(
            "photon_rate_mhz must be of size 1 (for constant value) or "
    ...
    for b in range(batch_size):
        n_photon = n_photon_from_rate(
            photon_rate_mhz=photon_rate_batch[b], n_bin=n_bin,
            bin_size_ns=bin_size_ns
        )
        waveform = waveform_from_n_photon(
            n_photon, template_amplitude_bin,
            n_bin_per_sample=n_bin_per_sample, noise_lsb=noise_lsb_batch[b]
        )
        if kernel is None:
            n_photon_batch[b, :] = n_photon[n_bin_init:]
        else:
            n_photon_smooth = np.convolve(n_photon, kernel, 'same')
            n_photon_batch[b, :] = n_photon_smooth[n_bin_init:]
        waveform_batch[b, :] = waveform[n_sample_init:]
    return waveform_batch, n_photon_batch


def generator_for_training(
        n_event=None, batch_size=1, n_sample=90, n_sample_init=20,
        photon_rate_mhz=100, bin_size_ns=0.5, sampling_rate_mhz=250,
        amplitude_gain=5., noise_lsb=1.05, sigma_smooth_photon_ns=0.
):
    """
    Generator returning for each iteration a batch of waveforms and a batch of
    photons. The MC simulation is done at each call, no repeating of events
    occurs.
    ...
    :param photon_rate_mhz: rate of photon to simulate. Can be a tuple, then
    for each event of all batches the rate is taken in the range given by the
    tuple.
    :param bin_size_ns: size of bins in nanoseconds for the photons array.
    ...
    for each event of all batches the rate is taken in the range given by the
    tuple.
    :param sigma_smooth_photon_ns: width of the gaussian kernel to
    convolve with the batch of photons. Use to convert the position of photon
    to a probability of with sigma_smooth_photon_ns. No convolution is done if
    sigma_smooth_photon_ns <= 0.
    :return: a tuple of 2 arrays at each iteration. First array is a batch of
    waveforms (amplitude of the sampled signal) and a batch of photons (number
    of photons per bin). For both, first dimension is the batch iteration,
    ...
    if sigma_smooth_photon_ns > 0:
        nbin_5sigma = int(np.round(sigma_smooth_photon_ns * 5 / bin_size_ns))
        t_kernel = np.arange(-nbin_5sigma, nbin_5sigma + .1, 1) * bin_size_ns
        gauss_kernel = np.exp(-0.5 * (t_kernel/ sigma_smooth_photon_ns) ** 2)
        gauss_kernel /= np.sum(gauss_kernel)
        print("gaussian kernel prepared, shape=", gauss_kernel.shape)
    if n_event is None:
        while True:
            waveform_batch, n_photon_batch = _get_batch(
                batch_size, n_sample, n_bin_per_sample, photon_rate_mhz,
                bin_size_ns, template_amplitude_bin, noise_lsb, n_sample_init,
                kernel=gauss_kernel
            )
            yield (waveform_batch, n_photon_batch)
    else:
...
def plot_example(
    ...
    generator = generator_for_training(
        ...
        photon_rate_mhz=photon_rate_mhz, bin_size_ns=bin_size_ns,
    ...
    for waveform_batch, n_photon_batch in generator:
        for waveform, n_photon_per_bin in zip(waveform_batch, n_photon_batch):
            t_max_ns = len(n_photon_per_bin) * bin_size_ns
            t_bin = np.arange(0, t_max_ns, bin_size_ns)
            t_waveform = np.arange(0, t_max_ns, 1000 / sampling_rate_mhz)
            ax1.plot(t_bin, n_photon_per_bin)
...
if __name__ == '__main__':
    plot_example(
        ...
        photon_rate_mhz=20, bin_size_ns=0.5, sampling_rate_mhz=250,
        amplitude_gain=5., noise_lsb=1.05
    )
